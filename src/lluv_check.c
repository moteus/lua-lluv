...
static int lluv_check_create(lua_State *L){
  ...
  uv_check_t *check   = (uv_check_t *)lluv_handle_create(L, UV_CHECK, INHERITE_FLAGS(loop));
  int err = uv_check_init(loop->handle, check);
  if(err < 0){
    lluv_handle_cleanup(L, (lluv_handle_t*)check->data);
...
static lluv_handle_t* lluv_check_check(lua_State *L, int idx, lluv_flags_t flags){
  ...
  luaL_argcheck (L, handle->handle->type == UV_CHECK, idx, LLUV_CHECK_NAME" expected");
...
static int lluv_check_start(lua_State *L){
  ...
  err = uv_check_start((uv_check_t*)handle->handle, lluv_on_check_start);
  if(err < 0){
...
static int lluv_check_stop(lua_State *L){
  lluv_handle_t *handle = lluv_check_check(L, 1, LLUV_FLAG_OPEN);
  int err = uv_check_stop((uv_check_t*)handle->handle);
  if(err < 0){
    return lluv_fail(L, handle->flags, LLUV_ERR_UV, err, NULL);
  }
  lua_settop(L, 1);
