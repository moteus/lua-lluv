...
static int lluv_fs_poll_create(lua_State *L){
  ...
  uv_fs_poll_t *fs_poll   = (uv_fs_poll_t *)lluv_handle_create(L, UV_FS_POLL, INHERITE_FLAGS(loop));
  int err = uv_fs_poll_init(loop->handle, fs_poll);
  if(err < 0){
    lluv_handle_cleanup(L, (lluv_handle_t*)fs_poll->data);
...
static lluv_handle_t* lluv_check_fs_poll(lua_State *L, int idx, lluv_flags_t flags){
  ...
  luaL_argcheck (L, handle->handle->type == UV_FS_POLL, idx, LLUV_FS_POLL_NAME" expected");
...
static int lluv_fs_poll_start(lua_State *L){
  ...
  err = uv_fs_poll_start((uv_fs_poll_t*)handle->handle, lluv_on_fs_poll_start, path, flags);
  if(err < 0){
...
static int lluv_fs_poll_stop(lua_State *L){
  lluv_handle_t *handle = lluv_check_fs_poll(L, 1, LLUV_FLAG_OPEN);
  int err = uv_fs_poll_stop((uv_fs_poll_t*)handle->handle);
  if(err < 0){
    return lluv_fail(L, handle->flags, LLUV_ERR_UV, err, NULL);
  }
  lua_settop(L, 1);
...
static int lluv_fs_getpath(lua_State *L){
  ...
  int err = uv_fs_poll_getpath((uv_fs_poll_t *)handle->handle, buf, &len);
  if(err >= 0){
  ...
  }
  {
    ...
    err = uv_fs_poll_getpath((uv_fs_poll_t *)handle->handle, buf, &len);
    if(err < 0){
      lluv_free(L, buf);
      return lluv_fail(L, handle->flags, LLUV_ERR_UV, err, NULL);
    }
